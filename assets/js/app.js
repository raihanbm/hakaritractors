...
function renderCategories(){
 categoryStrip.innerHTML=categories.slice(0,7).map(c=>`<button class="cat-card" data-cat="${esc(c.name)}"><span class="cat-icon">${icon(c.icon,18)}</span><span><b>${esc(c.name)}</b><small>${c.count} ${t("catalog entries")}</small></span></button>`).join("");
