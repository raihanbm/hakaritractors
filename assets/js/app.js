...
function renderExplodedSheet(sheet){
 ...
 if(usePdfViewer)setTimeout(()=>{const host=document.getElementById("sheet-pdf-host");if(host)new HikariDiagramPdfViewer({container:host,pdfUrl:sheet.pdf_url,crop:sheet.crop_config||null,showCropControls:false,onError:err=>{host.innerHTML=`<div class="sheet-canvas"><img src="${esc(sheet.full_image||sheet.preview_image||IMG.tractor)}" alt="${esc(sheet.title)} exploded diagram"><p style="color:var(--muted);font-size:10px;text-align:center;padding:20px">Diagram preview (PDF render failed)</p></div>`}})},50);
