...
test('public catalog is sourced from the downloaded Hikari Drive inventory, not generated demo SKUs', () => {
  ...
  assert.equal(catalog.importedPdfRecords, 1114);
  assert.equal(catalog.products.length, 1114);
  assert.ok(catalog.products.every((part) => part.priceEstimate === true));
  assert.equal(catalog.orderablePartRows, 5390);
  assert.equal(Object.keys(sheetIndex).length, 1114);
